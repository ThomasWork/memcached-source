...
static slabclass_t slabclass[MAX_NUMBER_OF_SLAB_CLASSES];//×Ü¹²ÓÐÕâÃ´¶àslabclass
...
//½«ÐÂ·ÖÅäµÄslab·Ö¸î³Éitem²¢³õÊ¼»¯ÀïÃæµÄÄÚÈÝ
...
static int do_slabs_newslab(const unsigned int id) {
    ...
    if ((mem_limit && mem_malloced + len > mem_limit && p->slabs > 0
         && g->slabs == 0)) {//ÕâÀïÅÐ¶ÏÊÇ·ñ»á³¬³ö¿Õ¼ä
...
static void do_slabs_free(void *ptr, const size_t size, unsigned int id) {
    ...
    it = (item *)ptr;
    if ((it->it_flags & ITEM_CHUNKED) == 0) {//ÔÚÕâÒ»²½Ö®Ç°ÐèÒªÉèÖÃit_flagså 
