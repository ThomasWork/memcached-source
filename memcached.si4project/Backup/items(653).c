...
item *do_item_alloc(char *key, const size_t nkey, const unsigned int flags,
    ...
    unsigned int id = slabs_clsid(ntotal);//ÕâÀïÎªÕÒµ½ºÏÊÊµÄclabclass£¬µ«ÊÇ×î´óµÄslabclassµÄ
    								//size¶¼Ğ¡ÓÚËùĞèµÄ¿Õ¼ä»áÔõÃ´°ì?
    								//chunkedÊı¾İÊÇÔõÃ´»ØÊÂ?
    if (id == 0)
        return 0;
     ...
     * and replacing small items.
     //Èç¹ûÃ»ÓĞ¿ÉÓÃÄÚ´æ
     */
    for (i = 0; i < 10; i++) {//Ñ­»·½øĞĞ10´Î
        uint64_t total_bytes;
        /* Try to reclaim memory first */// Ê×ÏÈ³¢ÊÔÈ¡»ØÄÚ´æ
        if (!settings.lru_maintainer_thread) {//Èç¹ûÃ»ÓĞÉèÖÃ¸Ã×Ö¶Î
            lru_pull_tail(id, COLD_LRU, 0, 0);//
        }
        it = slabs_alloc(ntotal, id, &total_bytes, 0);//ÕâÀï¼´ÎªÎªĞÂµÄÔªËØ·ÖÅä¿Õ¼ä£¬ntoal¿ÉÄÜ´óÓÚ¶ÔÓ¦id
        //µÄsize£¬ÄÇÃ´ĞèÒª·ÖÅächunked item  ?

        if (settings.expirezero_does_not_evict)
            total_bytes -= noexp_lru_size(id);

        if (it == NULL) {//Èç¹û·ÖÅäĞÂ¿Õ¼äÊ§°Ü£¬Ôò½øĞĞÏÂÃæµÄ²½Öè
    ...
    it->exptime = exptime;
    memcpy(ITEM_suffix(it), suffix, (size_t)nsuffix);//ÕâÀïnsuffixÒÑ¾­¸üĞÂÎªÊµ¼ÊĞèÒªµÄ´óĞ¡
    it->nsuffix = nsuffix;
...
//Ö»ÔÚÕâ¸öº¯ÊıÖĞµ÷ÓÃÁËassoc_insert
int do_item_link(item *it, const uint32_t hv) {
    ...
    assoc_insert(it, hv);//²åÈëµ½hash±íÖĞ
    item_link_q(it);//¼ÓËø²¢¼ÓÈëµ½lru¶ÓÁĞÖĞ
    refcount_incr(&it->refcount);//Ôö¼ÓÒıÓÃ¼ÆÊı£¬Äª·Ç¼ÓÈëµ½hash±íÖĞ¾ÍËãÒ»´ÎÒıÓÃ?
    item_stats_sizes_add(it);

    return 1;
}

//item_unlink ¼ÓËøÖ®ºóµ÷ÓÃ¸Ãº¯Êı£¬´Óhash±íÖĞÉ¾³ı£¬´ÓLRU¶ÓÁĞÉ¾³ı£¬¼ÓÈëslots
void do_item_unlink(item *it, const uint32_t hv) {
    MEMCACHED_ITEM_UNLINK(ITEM_key(it), it->nkey, it->nbytes);
    if ((it->it_flags & ITEM_LINKED) != 0) {//ÕâÀïÊÇÎªÁË·ÀÖ¹¶à´Îunlink?
        it->it_flags &= ~ITEM_LINKED;//ÉèÖÃitemÎª·Çlinked
        STATS_LOCK();
        stats_state.curr_bytes -= ITEM_ntotal(it);
        stats_state.curr_items -= 1;
        STATS_UNLOCK();
        item_stats_sizes_remove(it);
        assoc_delete(ITEM_key(it), it->nkey, hv);//´Óhash±íÖĞÉ¾³ı
        item_unlink_q(it);//´ÓLRU¶ÓÁĞÖĞÉ¾³ıÔªËØ
        do_item_remove(it);//Èç¹ûrefcount¼õµ½0£¬Ôò°ÑËü¹ÒÔØµ½slotsÖĞ
    }
}
...
void do_item_unlink_nolock(item *it, const uint32_t hv) {
    ...
    if ((it->it_flags & ITEM_LINKED) != 0) {
        ...
        item_stats_sizes_remove(it);
        assoc_delete(ITEM_key(it), it->nkey, hv);//´Óhash±íÖĞÉ¾³ı
        do_item_unlink_q(it);//´Ó LRU¶ÓÁĞÖĞÉ¾³ı
        do_item_remove(it);//Èç¹ûitemµÄrefcountµÈÓÚ0£¬½«itemÒÆ¶¯µ½slotsÖĞ£¬
    }
}

//Èç¹ûitemµÄÒıÓÃ¼ÆÊı¼õÉÙµ½0£¬½«Æä·Åµ½slabµÄslotsÖĞ
...
/* Bump the last accessed time, or relink if we're in compat mode *///µßô¤¡¢Åö×²£¬³å×²
//¸üĞÂ×îºó·ÃÎÊÊ±¼ä£¬»òÕßÖØĞÂÁ¬½ÓÈç¹ûÎÒÃÇÔÚ¼æÈİĞÔ Ä£Ê½
void do_item_update(item *it) {
    MEMCACHED_ITEM_UPDATE(ITEM_key(it), it->nkey, it->nbytes);
    if (it->time < current_time - ITEM_UPDATE_INTERVAL) {//Èç¹û¸üĞÂÊ±¼ä ´óÓÚ60Ãë
        assert((it->it_flags & ITEM_SLABBED) == 0);

        if ((it->it_flags & ITEM_LINKED) != 0) {//Èç¹ûËüÔÚÁ´±íÖĞ
            it->time = current_time;
            if (!settings.lru_maintainer_thread) {//lru ±£³ÖÏß³Ì×¨ÃÅµ÷ÕûË³Ğò?
                item_unlink_q(it);
                item_link_q(it);//´ÓÍ·²¿²åÈë
            }
        }
    }
}
...
int do_item_replace(item *it, item *new_it, const uint32_t hv) {
    ...
    do_item_unlink(it, hv);//Èı²½É¾³ı£¬²»¼ÓËø
    return do_item_link(new_it, hv);//²»¼ÓËøÌí¼Óµ½hash±íºÍLRU
}
 ...
 */
 //ÎÒÈÏÎªÈç¹û²»Ê¹ÓÃCASÖµ£¬Ã»ÓĞÊ²Ã´·½Ê½À´×¼È·Íê³ÉÕâ¸ö¹¦ÄÜ
 //
void item_stats_sizes_remove(item *it) {
    if (stats_sizes_hist == NULL || stats_sizes_cas_min > ITEM_get_cas(it))
        return;
    int ntotal = ITEM_ntotal(it);//item×Ü¹²ÓĞ¶àÉÙ×Ö½Ú
    int bucket = ntotal / 32;//ÊÇ32×Ö½ÚµÄ¶àÉÙ±¶
    if ((ntotal % 32) != 0) bucket++;//ÏòÉÏÈ¡ÕûÊı
    if (bucket < stats_sizes_buckets) stats_sizes_hist[bucket]--;
}
...
//¶Ôassoc_find ½øĞĞ°ü×°£¬ËüÄÚ²¿ÊµÏÖÁËÑÓ³Ù¹ıÆÚÂß¼­
item *do_item_get(const char *key, const size_t nkey, const uint32_t hv, conn *c) {
    item *it = assoc_find(key, nkey, hv);//Õâ¸öº¯Êı´¿´âÓÃÀ´´Óhash±íÖĞÕÒµ½ÔªËØ
    if (it != NULL) {
        refcount_incr(&it->refcount);//ÔÚ¸Ãº¯ÊıÖĞ£¬Èç¹ûÏµÍ³Ìá¹©Ô­×Ó²Ù×÷ÔòÀûÓÃ£¬·ñÔò¼ÓËø
        /* Optimization for slab reassignment. prevents popular items from
         * jamming in busy wait. Can only do this here to satisfy lock order
         * of item_lock, slabs_lock. */

//¶ÔslabÖØĞÂ·ÖÅä½øĞĞÓÅ»¯£¬×èÖ¹ÈÈÃÅitem ±»¼·½ø·±Ã¦µÄµÈ´ıÖĞ
//Ö»ÄÜÔÚÕâÀïÕâÑù×öÀ´Âú×ã¼ÓËøË³Ğò(ÏÈ item lock£¬È»ºóÊÇslabs_lock)
    ...
    int was_found = 0;//Õâ¸ö±äÁ¿ÓÃÀ´±£´æÊÇ·ñÕÒµ½ÔªËØÒÔ¼°¸ÃÔªËØÊÇ·ñ±»Çå¿ÕµÈ×´Ì¬
    ...
    if (it != NULL) {
        was_found = 1;
        if (item_is_flushed(it)) {//¸ù¾İoldesttimeºÍcasÅĞ¶ÏÊÇ·ñÇå¿Õ¸ÃÔªËØ
            do_item_unlink(it, hv);
            do_item_remove(it);//¿ÉÊÇ ÔÚdo_item_unlinkÖĞÒÑ¾­µ÷ÓÃÁËdo_item_remove
            it = NULL;
            pthread_mutex_lock(&c->thread->stats.mutex);
            c->thread->stats.get_flushed++;//¸üĞÂÏß³Ì×´Ì¬
            ...
            it = NULL;
            pthread_mutex_lock(&c->thread->stats.mutex);
            c->thread->stats.get_expired++;//¸üĞÂÏß³ÌµÄ×´Ì¬
            pthread_mutex_unlock(&c->thread->stats.mutex);
            if (settings.verbose > 2) {
 ...
 * Callable from worker threads or the LRU maintainer thread */
 //·µ»Ø±»ÒÆ³ıµÄ¡¢¹ıÆÚµÄ»òÕßÇıÖğ³öµÄÔªËØµÄÊıÁ¿
 //¿ÉÒÔ´ÓworkerÏß³Ì»òÕßLRU maintainerµ÷ÓÃ¸Ãº¯Êı
 //ÔÚcommand_updateÏà¹Øº¯ÊıÖĞµ÷ÓÃÁËÕâ¸öº¯ÊıÀ´ÇåÀí¿Õ¼ä?
static int lru_pull_tail(const int orig_id, const int cur_lru,
        const uint64_t total_bytes, uint8_t flags) {
    item *it = NULL;
    int id = orig_id;
    ...
    for (; tries > 0 && search != NULL; tries--, search=next_it) {
        ...
        if ((hold_lock = item_trylock(hv)) == NULL)//Èç¹û¼ÓËøÊ§°Ü£¬¼ÌĞøÑ­»·
            continue;
        /* Now see if the item is refcount locked */
        if (refcount_incr(&search->refcount) != 2) {//µ½ÕâÀï¼ÓËø³É¹¦£¬¶øÇÒÒıÓÃ´ÎÊı²»Îª2
        ...
        switch (cur_lru) {
            ...
            case WARM_LRU:
                ...
                break;
            case COLD_LRU://workerÏß³Ìµ÷ÓÃ¸Ãº¯Êı¾ÍÊÇCOLD_LRU
