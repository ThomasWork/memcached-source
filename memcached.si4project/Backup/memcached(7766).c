...
//Œƒº˛∑∂Œßƒ⁄µƒ±‰¡ø
static conn *listen_conn = NULL;//’˝‘⁄º‡Ã˝µƒ¡¨Ω”∂”¡–
static int max_fds;
static struct event_base *main_base;//÷˜—≠ª∑
 ...
 */
static volatile bool allow_new_conns = true;// «∑Ò‘ –Ì–¬µƒ¡¨Ω”Ω¯»Î£¨
									//µ±πÿ±’“ª∏ˆ¡¨Ω” ±£¨∏√÷µ±ª…Ë÷√Œ™’Ê
									//À˘“‘£¨»Áπ˚¡¨Ω” ˝ƒø¥ÔµΩ¡À◊Ó¥Û÷µ£¨∏√÷µŒ™ºŸÂ
 ...
 * be that low).
 */
 //¥´∏¯À¸µƒ≤Œ ˝“™√¥ «unix ±º‰£¨“™√¥ «æ‡¿Îµ±«∞ ±º‰µƒº‰∏Ù
 //∑µªÿunix ±º‰£¨ π”√µƒ“¿æ› «delta≤ªø…ƒ‹≥¨π˝“ª∏ˆ‘¬
...
static void *conn_timeout_thread(void *arg) {
    ...
    while(1) {
        ...
        for (i = 0; i < max_fds; i++) {
            ...
            if (!IS_TCP(c->transport))//»Áπ˚ «TCP¡¨Ω”
                continue;
...
//÷±µΩ–Ë“™¡¨Ω” ±Œ“√«≤≈∑÷≈‰¡¨Ω”ø’º‰
//µ´ «’‚¿ÔÃ·«∞∑÷≈‰¡Àƒ⁄¥Ê¿¥±£¥Ê÷∏’Î£¨
//”≈µ„ «ø…“‘ƒ‹πª÷±Ω” π”√FD¿¥À˜“˝¡¨Ω” ˝◊È
static void conn_init(void) {
    /* We're unlikely to see an FD much higher than maxconns. */
    int next_fd = dup(1);//∏¥÷∆≥…π¶ ±∑µªÿ◊Ó–°µƒ…–Œ¥±ª π”√µƒŒƒº˛√Ë ˆ∑˚
...
conn *conn_new(const int sfd, enum conn_states init_state,
    ...
    if (NULL == c) {//»Áπ˚Œ™ø’£¨±Ì æ
        ...
        MEMCACHED_CONN_CREATE(c);//∫Û√Êø™ º≥ı ºªØ¡¨Ω”–≈œ¢
        ...
        c->sfd = sfd;//’‚¿Ô»√¡¨Ω”∫ÕŒƒº˛√Ë ˆ∑˚∂‘”¶
        conns[sfd] = c;
    }

    c->transport = transport;
    c->protocol = settings.binding_protocol;//’‚∏ˆ◊÷∂Œ «∑Ò «√ø¥Œ–¬Ω®“ª∏ˆ¡¨Ω” ±∂º∏¸–¬?
    ...
    c->last_cmd_time = current_time; //µ±«∞ ±º‰Œ™“ª∏ˆ»´æ÷±‰¡ø£¨“ª√Î∏¸–¬“ª¥Œ /* initialize for idle kicker */
...
static void conn_cleanup(conn *c) {
    ...
    conn_release_items(c);//«Âø’¡¨Ω”¿Ô√Êµƒitems£¨µ´ «’‚–©itemsµΩµ◊”– ≤√¥”√Â
...
static void conn_close(conn *c) {
    ...
    event_del(&c->event);//…æ≥˝ ±º‰£¨ƒ«√¥¿¥◊‘∏√¡¨Ω”µƒœ˚œ¢∂ºΩ´ ’≤ªµΩ¡À

    if (settings.verbose > 1)
        fprintf(stderr, "<%d connection closed.\n", c->sfd);

    conn_cleanup(c);//«Âø’conn÷–µƒƒ⁄»›

    MEMCACHED_CONN_RELEASE(c->sfd);
    conn_set_state(c, conn_closed);//Ω´◊¥Ã¨…Ë÷√Œ™πÿ±’
    close(c->sfd);//πÿ±’æ‰±˙
 ...
 * buffers!
 */
 //»Áπ˚“ª∏ˆ¡¨Ω”µƒª∫¥ÊÃ´¥Û¡ÀæÕ ’ÀıÀ¸
 //’‚ƒ‹◊Ë÷π÷‹∆⁄–‘µƒ¥Ûµƒ"get"«Î«Û’º”√¥Û¡øµƒ∑˛ŒÒ∆˜ƒ⁄¥Ê
 ...
 * Returns 0 on success, -1 on out-of-memory.
 */
 //»∑±£‘⁄“ª∏ˆ¡¨Ω”µƒiov ¡–±Ì÷–”–¥Ê∑≈¡Ì“ª∏ˆiovecµƒø’º‰
 //»Áπ˚≥…π¶∑µªÿ0, »Áπ˚ƒ⁄¥Ê≤ª◊„∑µªÿ-1
static int ensure_iov_space(conn *c) {
    assert(c != NULL);
    ...
    if (c->iovused >= c->iovsize) {
        ...
        struct iovec *new_iov = (struct iovec *)realloc(c->iov,
                                (c->iovsize * 2) * sizeof(struct iovec));//√ø¥Œ∑≠±∂
        if (! new_iov) {
            STATS_LOCK();
            stats.malloc_fails++;
            STATS_UNLOCK();
...
static int add_iov(conn *c, const void *buf, int len) {
    ...
    do {
        ...
        if (ensure_iov_space(c) != 0)//‘ˆº”iov ˝◊Èµƒø’º‰
            return -1;
...
static void out_of_memory(conn *c, char *ascii_error) {
    ...
    if (c->protocol == binary_prot) {
        ...
        write_bin_error(c, PROTOCOL_BINARY_RESPONSE_ENOMEM, ascii_error, 0);//’‚¿Ôª·…Ë÷√◊¥Ã¨
    } else {
        out_string(c, ascii_error);//…Ë÷√◊¥Ã¨“‘º∞write_and_go
    }
}
 ...
 */
 //Œ“√«◊ﬂµΩ’‚“ª≤ΩÀµ√˜Œ“√«“—æ≠∂¡»°¡Àset/add/replace√¸¡Ó÷–µƒ÷µ°£
 //√¸¡Ó“—æ≠±ª¥Ê¥¢‘⁄c->cmd÷–£¨item“—æ≠¥Ê¥¢‘⁄c->item÷–
...
//œ¬√Ê≈–∂œis_valid «∑ÒŒ™’Ê
    if ((it->it_flags & ITEM_CHUNKED) == 0) {//±Ì æitem≤ª «◊È∫œµƒ
    ...
    if (!is_valid) {
      ...
      ret = store_item(it, comm, c);//////////////////////////////////////’‚¿Ô « µº µƒ“µŒÒ¬ﬂº≠
...
static void write_bin_response(conn *c, void *d, int hlen, int keylen, int dlen) {
    if (!c->noreply || c->cmd == PROTOCOL_BINARY_CMD_GET ||
        c->cmd == PROTOCOL_BINARY_CMD_GETK) {//‘⁄’‚÷÷«Èøˆœ¬–Ë“™ªÿ∏¥–≈œ¢
 ...
 */
 //‘⁄store_item∫Ø ˝÷–“—æ≠∂‘itemº”À¯¡À£¨»ª∫Û≤≈µ˜”√µƒÀ¸
enum store_item_type do_store_item(item *it, int comm, conn *c, const uint32_t hv) {
    char *key = ITEM_key(it);
    item *old_it = do_item_get(key, it->nkey, hv, c);//¥”hash±Ì÷–≤È’“‘™Àÿ£¨»Áπ˚π˝∆⁄£¨…Ë÷√Œ™NULL
    enum store_item_type stored = NOT_STORED;

    item *new_it = NULL;
    uint32_t flags;

    if (old_it != NULL && comm == NREAD_ADD) {//∏√‘™Àÿ¥Ê‘⁄≤¢«“√¸¡Ó «ÃÌº”‘™Àÿ
        /* add only adds a nonexistent item, but promote to head of LRU */
        do_item_update(old_it);//∏¸–¬itemµƒ ±º‰£¨≤¢∞—À¸“∆µΩ∂”¡–ƒ©Œ≤
    } else if (!old_it && (comm == NREAD_REPLACE//»Áπ˚‘™Àÿ≤ª¥Ê‘⁄
        || comm == NREAD_APPEND || comm == NREAD_PREPEND))
    {
        ...
        if(old_it == NULL) {
            ...
            stored = NOT_FOUND;
            pthread_mutex_lock(&c->thread->stats.mutex);
            c->thread->stats.cas_misses++;//∏¸–¬œﬂ≥Ã÷–µƒ’“≤ªµΩººº∆ ˝
            pthread_mutex_unlock(&c->thread->stats.mutex);
        }
        else if (ITEM_get_cas(it) == ITEM_get_cas(old_it)) {//’“µΩ÷Æ«∞¥Ê‘⁄µƒ£¨≤¢«“cas Ù–‘“ª÷¬
            // cas validates
            // it and old_it may belong to different classes.
            // I'm updating the stats for the one that's getting pushed out
            pthread_mutex_lock(&c->thread->stats.mutex);
            c->thread->stats.slab_stats[ITEM_clsid(old_it)].cas_hits++;//∏¸–¬œﬂ≥ÃÕ≥º∆–≈œ¢
            pthread_mutex_unlock(&c->thread->stats.mutex);

            item_replace(old_it, it, hv);//ÃÊªª≤Ÿ◊˜?????????????????????????????????’‚¿Ô÷¥––µƒ µº …œ «ÃÊªª?
            stored = STORED;
        } else {//’“µΩ¡À£¨µ´ «¡Ω’ﬂcas Ù–‘≤ª“ª÷¬£¨À˘“‘Ω¯––º∆ ˝∫Û ≤√¥∂º≤ª◊ˆ
            pthread_mutex_lock(&c->thread->stats.mutex);
    ...
    if (old_it != NULL)
        do_item_remove(old_it);   //Ω´∆‰π“‘ÿ µΩslots÷–
    if (new_it != NULL)
...
static inline void process_get_command(conn *c, token_t *tokens, size_t ntokens, bool return_cas) {
    ...
    do {
        while(key_token->length != 0) {
            ...
            if(nkey > KEY_MAX_LENGTH) {
                out_string(c, "CLIENT_ERROR bad command line format");
                while (i-- > 0) {
                    item_remove(*(c->ilist + i));//«Âø’¡¨Ω”¿Ô√Ê±£¥Êµƒitem
                }
                return;
            }
            ...
            if (it) {
                ...
                else
                {
                  ...
                  if ((it->it_flags & ITEM_CHUNKED) == 0)//≤ª «chunked ˝æ›
                      {
				...
				//item_get‘ˆº”¡Àit->refcount
                pthread_mutex_lock(&c->thread->stats.mutex);
                c->thread->stats.slab_stats[ITEM_clsid(it)].get_hits++;
                c->thread->stats.get_cmds++;
                pthread_mutex_unlock(&c->thread->stats.mutex);
                item_update(it);//////////////////////////////////////////∏¸–¬itemµƒ∑√Œ  ±º‰£¨∑«∆µ∑±∏¸–¬
                *(c->ilist + i) = it;
                i++;
    ...
    */
    //»Áπ˚—≠ª∑±ªƒ⁄¥Ê≤ª◊„÷–∂œ£¨œÚbuffer÷–º”»Î Œ≤≤øµƒ\r\n
    // «≤ªø…øøµƒ£¨“ÚŒ™À¸ø…ƒ‹≤ª «“‘\r\nΩ·Œ≤
    //“Ú¥ÀŒ“√«∑¢ÀÕSERVER_ERROR
...
static void process_update_command(conn *c, token_t *tokens, const size_t ntokens, int comm, bool handle_cas) {
    ...
    set_noreply_maybe(c, tokens, ntokens);//…Ë÷√ «∑Ò–Ë“™ªÿ∏¥Â
    ...
    if (! (safe_strtoul(tokens[2].value, (uint32_t *)&flags)//∂¡»°flags
           && safe_strtol(tokens[3].value, &exptime_int)//∂¡»°exptime_int
           && safe_strtol(tokens[4].value, (int32_t *)&vlen))) {//∂¡»°vlen
        out_string(c, "CLIENT_ERROR bad command line format");
        return;
    }
    ...
    vlen += 2;//’‚¿Ôº”¡À2£¨ «Œ™ ≤√¥ƒÿ?Œ™¡À∂‡∑÷≈‰“ªµ„ ˝æ›?

    if (settings.detail_enabled) {
        stats_prefix_record_set(key, nkey);
    }

    it = item_alloc(key, nkey, flags, realtime(exptime), vlen);/////////////////////’‚¿Ô∑÷≈‰¡Àø’º‰
    											//µ´ « µº …œ ˝æ›∫√œÒ√ª”–¥Ê¥¢Ω¯»•

    if (it == 0) {
        enum store_item_type status;
        if (! item_size_ok(nkey, flags, vlen)) {//ƒø±ÍÃ´¥Û¡À
        ...
        /* swallow the data line */
        c->write_and_go = conn_swallow;//////////////////////////////////////////////////////◊¥Ã¨◊™ªª
        c->sbytes = vlen;

        /* Avoid stale data persisting in cache because we failed alloc.
         * Unacceptable for SET. Anywhere else too? */
        if (comm == NREAD_SET) {//»Áπ˚√¸¡Ó «set
            it = item_get(key, nkey, c);
            if (it) {
                item_unlink(it);//»Áπ˚≤È’“µΩ∏√‘™Àÿ£¨‘Ú∞—À¸…æ≥˝
                item_remove(it);//«∞“ª∏ˆ∫Ø ˝“—æ≠µ˜”√¡Àdo_item_remove£¨Œ™ ≤√¥ «2¥Œƒÿ?
            }
        }
...
static void process_command(conn *c, char *command) {
    ...
    c->iovused = 0;
    if (add_msghdr(c) != 0) {
        out_of_memory(c, "SERVER_ERROR out of memory preparing response");//ƒ⁄¥Ê¥ÌŒÛ£¨(œ»–¥)»ª∫ÛµΩnew_conn◊¥Ã¨
        return;
    }

    ntokens = tokenize_command(command, tokens, MAX_TOKENS);//Ω´√¸¡Ó¥Æ∞¥ø’∏Ò∑÷∏Ó
    if (ntokens >= 3 &&
        ((strcmp(tokens[COMMAND_TOKEN].value, "get") == 0) ||// COMMAND_TOKEN  µº …œµ»”⁄0
         (strcmp(tokens[COMMAND_TOKEN].value, "bget") == 0))) {

        process_get_command(c, tokens, ntokens, false);//¥¶¿Ì                           get                        √¸¡Ó
        ...
        process_update_command(c, tokens, ntokens, comm, false);// ¥¶¿Ì      update                  √¸¡Ó
        ...
        if (ntokens != (c->noreply ? 3 : 2)) {
            exptime = strtol(tokens[1].value, NULL, 10);//π˝∆⁄ ±º‰
            if(errno == ERANGE) {
                out_string(c, "CLIENT_ERROR bad command line format");
                return;
            }
        ...
        */
        //»Áπ˚exptime µ»”⁄0£¨ƒ«√¥realtime() “≤ª·∑µªÿ0
        //ƒ«√¥0-1ª·“Á≥ˆµ√µΩ◊‘¥ÛµƒŒﬁ∑˚∫≈÷µ
        //
        if (exptime > 0) {
            new_oldest = realtime(exptime);
        } else { /* exptime == 0 */
...
static enum try_read_result try_read_udp(conn *c) {
    ...
    res = recvfrom(c->sfd, c->rbuf, c->rsize,
                   ...
                   &c->request_addr_size);
    if (res > 8) {//’‚¿ÔŒ™ ≤√¥“™¥Û”⁄8ƒÿ£¨8 «ÀÊ±„÷∏∂®µƒ“ª∏ˆ¥Û”⁄0µƒ ˝◊÷Â
 ...
 */
 //æ°ø…ƒ‹µÿ∂‡¥”Õ¯¬Á÷–∂¡»°£¨¥¶¿Ìª∫¥Ê“Á≥ˆ“‘º∞¡¨Ω”πÿ±’
 //‘⁄∂¡»°÷Æ«∞£¨∞—“ª∏ˆ√¸¡Óµƒ £œ¬µƒ∂Œ“∆∂ØµΩª∫¥ÊµƒÕ∑≤ø
 //Œ™¡À±£ª§Œ“√«√‚ ‹ƒ≥»À π”√Œ±‘Ïµƒ ˝æ›flooding¡¨Ω”£¨‘Ï≥…¡¨Ω”≥‘µÙÀ˘”–ø…”√ƒ⁄¥Ê
 //£¨Ã¯≥ˆ≤¢«“‘⁄º∏¥Œrealloc ÷Æ∫Ûø™ ººÏ≤È∏’∏’Ω” ’µΩµƒ ˝æ›
static enum try_read_result try_read_network(conn *c) {
    ...
    if (c->rcurr != c->rbuf) {
        if (c->rbytes != 0) /* otherwise there's nothing to copy */
            memmove(c->rbuf, c->rcurr, c->rbytes);//ø…“‘¥¶¿Ìƒ⁄¥Ê∏≤∏«µƒ«Èøˆ£¨dest, src, n
        c->rcurr = c->rbuf;
    }

    while (1) {
        if (c->rbytes >= c->rsize) {
            if (num_allocs == 4) {//»Áπ˚“—æ≠¥ÔµΩ∑÷≈‰¥Œ ˝…œœﬁ
                return gotdata;
            }
            ++num_allocs;
            char *new_rbuf = realloc(c->rbuf, c->rsize * 2);//∑÷≈‰∏¸¥Ûµƒø’º‰
            if (!new_rbuf) {
                STATS_LOCK();
                stats.malloc_fails++;
                STATS_UNLOCK();
                ...
                c->write_and_go = conn_closing;////////◊¢“‚’‚¿Ô”–“ª∏ˆ◊¥Ã¨◊™±‰
                return READ_MEMORY_ERROR;
            }
            c->rcurr = c->rbuf = new_rbuf;//rcurr ÷ª «“ª∏ˆ÷∏’Î£¨÷∏œÚµ±«∞ª∫¥Êµƒƒ≥∏ˆŒª÷√
            c->rsize *= 2;
        }

        int avail = c->rsize - c->rbytes;//ø…“‘∂¡»Î’‚√¥∂‡ƒ⁄»›
        res = read(c->sfd, c->rbuf + c->rbytes, avail);
        if (res > 0) {
            pthread_mutex_lock(&c->thread->stats.mutex);//’‚¿Ô∏¸–¬œﬂ≥Ãƒ⁄≤ø◊¥Ã¨ «∑Ò”–±ÿ“™º”À¯ƒÿÂ
            c->thread->stats.bytes_read += res;
            pthread_mutex_unlock(&c->thread->stats.mutex);
 ...
 *   TRANSMIT_HARD_ERROR Can't write (c->state is set to conn_closing)
 */
 //¥”Œ“√«µƒ–≈œ¢ª∫¥ÊΩ·ππ÷–¥´ ‰œ¬“ªøÈ ˝æ›

static enum transmit_result transmit(conn *c) {
    assert(c != NULL);

    if (c->msgcurr < c->msgused &&
            c->msglist[c->msgcurr].msg_iovlen == 0) {//µ±«∞œ˚œ¢–¥ÕÍ±œ£¨Õ∆ºˆµΩœ¬“ª∏ˆœ˚œ¢
        /* Finished writing the current msg; advance to the next. */
        c->msgcurr++;//
    }
    if (c->msgcurr < c->msgused) {
        ssize_t res;
        struct msghdr *m = &c->msglist[c->msgcurr];//÷∏œÚµ±«∞µƒmsg

        res = sendmsg(c->sfd, m, 0);//∑¢ÀÕœ˚œ¢
        if (res > 0) {//∑¢ÀÕ≥…π¶
            pthread_mutex_lock(&c->thread->stats.mutex);
            ...
            /* We've written some of the data. Remove the completed
               ...
               //Œ“√«“—æ≠–¥¡À≤ø∑÷ ˝æ›£¨¥”µ»¥˝–¥µƒ¡–±Ì÷–“∆≥˝ÕÍ≥…µƒiovec µÃÂ
...
static void drive_machine(conn *c) {
    ...
    while (!stop) {
        ...
        switch(c->state) {
        case conn_listening://º¥ π≤ª∂¡»°£¨ƒ«√¥œ¬¥Œ“≤ª·∂¡»°µƒ
            addrlen = sizeof(addr);
#ifdef HAVE_ACCEPT4
            if (use_accept4) {
            ...
            if (sfd == -1) {
                ...
                if (errno == EAGAIN || errno == EWOULDBLOCK) {
                    /* these are transient, so don't log anything *///∂Ã‘›µƒ£¨¡Ÿ ±µƒ
                    stop = true;
        ...
        case conn_waiting:
            if (!update_event(c, EV_READ | EV_PERSIST)) {//∏¸–¬ ¬º˛£¨»Áπ˚“ª÷¬‘Ú≤ª”√∏¸–¬
                                                                                      //’‚¿Ô”¶∏√÷˜“™ «Œ™¡À∑¥¿°ƒ«∏ˆ ¬º˛
                if (settings.verbose > 0)
                    fprintf(stderr, "Couldn't update event\n");
                conn_set_state(c, conn_closing);
            ...
            conn_set_state(c, conn_read);
            stop = true;//’‚¿ÔstopŒ™true£¨ª·Ã¯≥ˆ»•£¨µ´ «∏√º‡Ã˝ ¬º˛»‘»ª≥…¡¢
            break;
        ...
        case conn_read:
            ...
            switch (res) {
            case READ_NO_DATA_RECEIVED://µ⁄“ª¥Œ∂¡»°∑¢…˙÷–∂œø…ƒ‹ª·’‚—˘
                conn_set_state(c, conn_waiting);
                break;
            case READ_DATA_RECEIVED:
                conn_set_state(c, conn_parse_cmd);
                break;
            case READ_ERROR://”¶∏√ «πÿ±’¡¨Ω”
                conn_set_state(c, conn_closing);
                break;
        ...
        case conn_parse_cmd ://◊¢“‚£¨’‚¿Ôœ¬“ª≤Ω—≠ª∑»‘»ªºÃ–¯Ω¯––
        ...
        case conn_new_cmd://◊¢“‚£¨’‚¿Ô√ª”–…Ë÷√Œ™true£¨À˘“‘—≠ª∑ª·ºÃ–¯Ω¯––œ¬»•
            /* Only process nreqs at a time to avoid starving other
               connections */
               //“ª¥ŒΩˆΩˆ¥¶¿ÌnÃı«Î«Û£¨“‘∑¿÷π∆‰À˚¡¨Ω”º¢∂ˆ

            --nreqs;
            if (nreqs >= 0) {
                reset_cmd_handler(c);//ªπø…“‘ÕÍ≥…≤ø∑÷«Î«Û
            } else {//»Áπ˚«Î«Û÷¥––ÕÍ≥…
                pthread_mutex_lock(&c->thread->stats.mutex);
                ...
                if (c->rbytes > 0) {
                    ...
                    */
                    if (!update_event(c, EV_WRITE | EV_PERSIST)) {//ƒ«√¥”¶∏√ª·≈≈‘⁄∂”¡–Œ≤≤ø£¨
                        if (settings.verbose > 0)                                     //œ¬¥Œ¥•∑¢–¥ ¬º˛ ±»‘»ªø…“‘¥¶¿Ì√¸¡Ó
                                                                                                  //µ´ «œ¬¥Œµ√’“ª˙ª·∏ƒπ˝¿¥
                            fprintf(stderr, "Couldn't update event\n");
                        conn_set_state(c, conn_closing);
                        break;
        ...
        case conn_nread://’‚¿Ô”¶∏√“—æ≠ø™ º÷¥––√¸¡Ó¡À
            if (c->rlbytes == 0) {//’‚¿Ô”¶∏√Àµµƒ «: »Áπ˚√ª”––Ë“™∂¡»°µƒƒ⁄»›
                complete_nread(c);
                break;
            }
		...
		//»Áπ˚itemŒ™ø’£¨ªÚ’ﬂ ˝æ›≤ª «chunked
            if (!c->item || (((item *)c->item)->it_flags & ITEM_CHUNKED) == 0) {
                /* first check if we have leftovers in the conn_read buffer */
                if (c->rbytes > 0) {
                    int tocopy = c->rbytes > c->rlbytes ? c->rlbytes : c->rbytes;//–Ë“™øΩ±¥µƒ ˝æ›≥§∂»
                    if (c->ritem != c->rcurr) {
                        memmove(c->ritem, c->rcurr, tocopy);//ritem µº ÷∏œÚ¥Ê¥¢ ˝æ›µƒø’º‰
                    }
                    c->ritem += tocopy;//“∆∂Øµ±«∞÷∏’Î
                    c->rlbytes -= tocopy;//–Ë“™∂¡»°µƒ≥§∂»∏¸–¬
                    c->rcurr += tocopy;
                    c->rbytes -= tocopy;
                    if (c->rlbytes == 0) {//»Áπ˚ ˝æ›ƒ⁄»›∂¡»°ÕÍ±œ
                        break;
                    }
                }

		//ª∫¥Ê÷–µƒ ˝æ›∂¡»°ÕÍ±œ£¨œ÷‘⁄ø™ º¥”socket÷–∂¡»°
                ...
                if (res > 0) {
                    ...
                    c->ritem += res;
                    c->rlbytes -= res;
                    break;///’‚¿ÔÃ¯≥ˆ»•¡À£¨µ´ «»Áπ˚ ˝æ›√ª”–∂¡ÕÍª·≥ˆ∑¢ EAGAIN
                }
            } else {
                res = read_into_chunked_item(c);/////////////∂¡»°chunked    item
                if (res > 0)
                    break;
            }

            if (res == 0) { /* end of stream */         //’‚¿Ô”¶∏√ « ˝æ›∂¡»°ÕÍ±œ¡À£¨µ´ «√ª”–÷¥––√¸¡Ó¬
                conn_set_state(c, conn_closing);
                break;
            }

            if (res == -1 && (errno == EAGAIN || errno == EWOULDBLOCK)) {//»Áπ˚∂¡»°±ª÷–∂œ£¨Ã¯≥ˆ—≠ª∑
                ...
                stop = true;//’‚¿Ô±Ì æÕ£÷π£¨µ´ «œ¬¥Œ»‘»ªª·±ªªΩ–—
                break;
            }
	...
	// ◊œ»ºÏ≤È‘⁄ª∫¥Ê÷–µƒ ˝æ›
        ...
        case conn_write:
             ...
             */
             //Œ“√«œÎœÚÕ‚–¥“ª∏ˆºÚµ•µƒªÿ”¶£¨»Áπ˚Œ“√«ªπ√ª”–ÕÍ≥…
             //ƒ«√¥æÕ∞—À¸◊È◊∞Ω¯“ª∏ˆœ˚œ¢ª∫¥Ê¡–±Ì
            if (c->iovused == 0 || (IS_UDP(c->transport) && c->iovused == 1)) {
                if (add_iov(c, c->wcurr, c->wbytes) != 0) {//’‚¿Ô÷˜“™ ««£…ÊµΩµ◊≤„µƒ µœ÷
                    if (settings.verbose > 0)
          ...
          if (IS_UDP(c->transport) && c->msgcurr == 0 && build_udp_headers(c) != 0) {
            ...
            case TRANSMIT_COMPLETE:
                if (c->state == conn_mwrite) {
                    ...
                    if(c->protocol == binary_prot) {
                        conn_set_state(c, c->write_and_go);//Œ™ ≤√¥∂˛Ω¯÷∆–≠“È“™’‚—˘ƒÿ?
                    } else {
                        conn_set_state(c, conn_new_cmd);
                    }
            ...
            case TRANSMIT_INCOMPLETE://√ª”–∞—“ªÃıœ˚œ¢–¥ÕÍ
            case TRANSMIT_HARD_ERROR:
                break;                   //‘⁄◊¥Ã¨ª˙÷–ºÃ–¯÷¥––

            case TRANSMIT_SOFT_ERROR://Õ£÷π◊¥Ã¨ª˙
                stop = true;
                break;
            }
            break;//‘Ÿ¥ŒΩ¯»Î—≠ª∑
        ...
        case conn_closed:
            /* This only happens if dormando is an idiot. */ //»Áπ˚dormando «∏ˆ…µπœ
            abort();
            break;
 ...
 *        listen on.
 */
 //¥¥Ω®“ª∏ˆsocket ≤¢∞—À¸∞Û∂®µΩÃÿ∂®µƒ∂Àø⁄∫≈
static int server_socket(const char *interface,
                         int port,
                         enum network_transport transport,//¥´ ‰–≠“È
                         FILE *portnumber_file) {//“ª∏ˆŒƒº˛£¨”√¿¥–¥∂Àø⁄∫≈¬Î
    int sfd;
...
//…Ë÷√µ˜”√close»‘»ªø…“‘÷ÿ–¬ π”√∏√socket
        setsockopt(sfd, SOL_SOCKET, SO_REUSEADDR, (void *)&flags, sizeof(flags));//µΩ’‚¿Ô”¶∏√∂º «1
        if (IS_UDP(transport)) {
            maximize_sndbuf(sfd);
        } else {//…Ë÷√ TCP  Ù–‘
            ...
            error = setsockopt(sfd, SOL_SOCKET, SO_LINGER, (void *)&ling, sizeof(ling));//æ≠¿˙time_waitµƒπ˝≥Ã
            if (error != 0)
                perror("setsockopt");
        ...
        if (bind(sfd, next->ai_addr, next->ai_addrlen) == -1) {
            ...
            success++;
            if (!IS_UDP(transport) && listen(sfd, settings.backlog) == -1) {//ø™ ºTCP  º‡Ã˝£¨’‚¿Ô «µ»”⁄-1
                perror("listen()");
                close(sfd);
                freeaddrinfo(ai);
                return 1;
        ...
        if (IS_UDP(transport)) {
            ...
            if (!(listen_conn_add = conn_new(sfd, conn_listening,//–¬Ω®¡¨Ω”∂‘œÛ£¨≤¢∞—À¸∞Û∂®µΩbase÷–
                                             EV_READ | EV_PERSIST, 1,
                                             transport, main_base))) {
                fprintf(stderr, "failed to create listening connection\n");
                exit(EXIT_FAILURE);
            }
            listen_conn_add->next = listen_conn;//º”»Î¡¨Ω”∂”¡–
...
static int server_sockets(int port, enum network_transport transport,
    ...
    if (settings.inter == NULL) {
        ...
        char *b;
        int ret = 0;
        char *list = strdup(settings.inter);//øΩ±¥◊÷∑˚¥Æ£¨◊Ó∫Û–Ë“™ π”√free Õ∑≈
 ...
 */
 //Œ“√« ‘⁄»´æ÷±‰¡ø÷–±£¥Êµ±«∞ ±º‰£¨≤¢«“ π”√“ª∏ˆº∆ ± ¬º˛∏¸–¬À¸°£
 //’‚Œ™Œ“√«Ω⁄ °¡À∫‹∂‡ ±º‰£¨“ÚŒ™Œ“√« µº …œ÷ª–Ë“™1√Î÷”∏¸–¬“ª¥Œ ±º‰°£
volatile rel_time_t current_time;
static struct event clockevent;

//libevent π”√µ•µ˜ ±º‰Ω¯–– ¬º˛µ˜∂»£¨≥˝¡Àjitter£
/* libevent uses a monotonic clock when available for event scheduling. Aside
 * from jitter, simply ticking our internal timer here is accurate enough.
 * Note that users who are setting explicit dates for expiration times *must*
 * ensure their clocks are correct before starting memcached. */
 //«Î◊¢“‚»Áπ˚”√ªßŒ™π˝∆⁄ ±º‰…Ë÷√√˜»∑µƒ»’∆⁄£¨
 //±ÿ–Î‘⁄∆Ù∂Ømemcached÷Æ«∞»∑±£À˚√«µƒ ±÷” «◊º»∑µƒ
static void clock_handler(const int fd, const short which, void *arg) {
    struct timeval t = {.tv_sec = 1, .tv_usec = 0};//1√Î÷”∏¸–¬“ª¥Œ
    ...
    evtimer_set(&clockevent, clock_handler, 0);//…Ë÷√ ¬º˛
    event_base_set(main_base, &clockevent);//…Ë÷√”Î÷Æπÿ¡™µƒbase
    evtimer_add(&clockevent, &t);//ÃÌº” ¬º˛
...
int main (int argc, char **argv) {
    ...
    /* handle SIGINT and SIGTERM *///∂‘–≈∫≈Ω¯––¥¶¿Ì
    signal(SIGINT, sig_handler);
    signal(SIGTERM, sig_handler);

    /* init settings */
    settings_init();//∂‘‘À––µƒ≤Œ ˝Ω¯––≥ı ºªØ
...
//…Ë÷√hashÀ„∑®
     ...
     * multiple ports
     */
     //»Áπ˚”√ªß÷∏∂®¡À∂‡∏ˆ∂Àø⁄£¨‘Ú¡Ó“ª∏ˆπ§◊˜œﬂ≥Ã∑˛ŒÒ√ø“ª∏ˆUDP∂Àø⁄
...
//»Áπ˚”––Ë“™£¨∞—◊ ‘¥œﬁ÷∆‘ˆº”¿¥‘ –ÌΩ” ’∏¸∂‡¡¨Ω”
	...
	//»Áπ˚“™«Û◊˜Œ™ ÿª§Ω¯≥Ã
	//»Áπ˚œÎ»∑±£”–dump coreµƒƒ‹¡¶£¨≤ª“™Ω´µ±«∞ƒø¬º±‰∏¸Œ™/
    if (do_daemonize) {
        if (sigignore(SIGHUP) == -1) {
            perror("Failed to ignore SIGHUP");
        }
        if (daemonize(maxcore, settings.verbose) == -1) {//¥¥Ω® ÿª§Ω¯≥Ã
    ...
    if (lock_memory) {
#ifdef HAVE_MLOCKALL
        int res = mlockall(MCL_CURRENT | MCL_FUTURE);//À¯∂®ƒ⁄¥Ê£¨Ω˚÷πΩ¯––Ωªªª
        if (res != 0) {
    ...
    main_base = event_init();//≥ı ºªØ÷˜libeventœﬂ≥Ã---------------------------------------
    ...
    slabs_init(settings.maxbytes, settings.factor, preallocate,
     ...
     //∫ˆ¬‘PIPE–≈∫≈£¨PIPE–≈∫≈ «µ±Õ¯¬Á¡¨Ω”“ª∂À“—æ≠∂œø™£¨
     //’‚ ±∑¢ÀÕ ˝æ›£¨ª·Ω¯––RSTµƒ÷ÿ∂®œÚ£¨‘Ÿ¥Œ∑¢ÀÕ ˝æ›£¨
     //ª·¥•∑¢PIPE–≈∫≈£¨∂¯PIPE–≈∫≈µƒƒ¨»œ∂Ø◊˜ «ÕÀ≥ˆΩ¯≥Ã£¨
     //À˘“‘–Ë“™∫ˆ¬‘∏√–≈∫≈
    ...
    if (start_assoc_maintenance_thread() == -1) {//ø™∆Ù¿©»›œﬂ≥Ã
        exit(EXIT_FAILURE);
    }

    if (start_lru_crawler && start_item_crawler_thread() != 0) {//’‚∏ˆœﬂ≥Ã «”√¿¥◊ˆ ≤√¥µƒÂ
        fprintf(stderr, "Failed to enable LRU crawler thread\n");
        exit(EXIT_FAILURE);
    }

    if (start_lru_maintainer && start_lru_maintainer_thread() != 0) {//LRUœﬂ≥Ã «”√¿¥◊ˆ ≤√¥µƒÂ
        fprintf(stderr, "Failed to enable LRU maintainer thread\n");
        return 1;
    }

    if (settings.slab_reassign &&
        start_slab_maintenance_thread() == -1) {//’‚∏ˆœﬂ≥Ã «”√¿¥?
        exit(EXIT_FAILURE);
    }

    if (settings.idle_timeout && start_conn_timeout_thread() == -1) {//¡¨Ω”≥¨ ±ºÏ≤‚œﬂ≥Ã
        exit(EXIT_FAILURE);
    }

    clock_handler(0, 0, 0);//≥ı ºªØº∆ ± ¬º˛£¨’‚—˘ ¬º˛√ø√Î÷”∏¸–¬“ª¥Œ£¨∑¿÷πœµÕ≥µ˜”√
     ...
     * is only an advisory.//πÀŒ µƒ£¨»∞∏Êµƒ
     */
    usleep(1000);
    if (stats_state.curr_conns + stats_state.reserved_fds >= settings.maxconns - 1) {//±£¡Ù√Ë ˆ∑˚
        fprintf(stderr, "Maxconns setting is too low, use -c to increase.\n");
        exit(EXIT_FAILURE);
    }

    if (pid_file != NULL) {
        save_pid(pid_file);//±£¥ÊΩ¯≥Ãid?
    }
