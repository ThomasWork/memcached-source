...
typedef struct {
    unsigned int size;   //Ã¿Ò»¸öitemµÄ´óÐ¡   /* sizes of items */
    unsigned int perslab;  //Ã¿¸öslab ÓÐ¶àÉÙitem  /* how many items per slab */

    void *slots;     //itemÖ¸Õë¹¹³ÉµÄË«ÏòÁ´±í£¬¿ÕÏÐÁ´±í      /* list of item ptrs */
    unsigned int sl_curr;  //¿ÕÏÐÁ´±íÖÐitem ×ÜÊý

    unsigned int slabs; //Õâ¸öclass ±»·ÖÅä¶àÉÙslab    /* how many slabs were allocated for this class */

//ÔÚslabclass[0] ÖÐÔ¤ÏÈ·ÖÅäÁËÒ»Ð©page(slab)£¬È»ºóget_page_from_global_pllÖÐ·µ»ØÒ»¸öpage
    void **slab_list;  //slab Ö¸ÕëµÄÊý×é     /* array of slab pointers */
    unsigned int list_size; //slab_listÊý×éµÄ´óÐ¡£¬Êµ¼ÊÉÏ´æ·ÅÊý¾ÝµÄÐ¡ÓÚlist_size /* size of prev array */

    size_t requested; //±»ÇëÇóµÄ×Ö½ÚÊý£¬ÓÃÀ´Í³¼ÆÊ¹ÓÃÂÊ? /* The number of requested bytes */
} slabclass_t;

static slabclass_t slabclass[MAX_NUMBER_OF_SLAB_CLASSES];//slabclassÊý×é(63 + 1)
static size_t mem_limit = 0;//×ÜµÄÄÚ´æÏÞÖÆ?
static size_t mem_malloced = 0;
/* If the memory limit has been hit once. Used as a hint to decide when to
 * early-wake the LRU maintenance thread */
static bool mem_limit_reached = false;//ÅÐ¶ÏÄÚ´æÏÞÖÆÊÇ·ñ´ïµ½¡£±»ÓÃÀ´×÷ÎªÒ»¸öÌáÊ¾£¬»½ÐÑLRU Ïß³Ì
static int power_largest;//µÈÓÚslabclassµÄÊýÁ¿

//slabs.c
static void *mem_base = NULL;//Ô¤·ÖÅäÒ»¸ö´óµÄ¿Õ¼ä£¬ÔÚºóÐøÇëÇóÐÂµÄ¿Õ¼äÊ±£¬´Ó¸Ã¿Õ¼ä»ñÈ¡£¬Èç¹ûÎª·ÖÅäÔò´ÓÏµÍ³ÉêÇë
static void *mem_current = NULL;//mem_baseÖ±ÏßµÄÔ¤·ÖÅä¿Õ¼äÖÐ¿ÕÏÐÄÚ´æ¿ªÊ¼Î»ÖÃ
static size_t mem_avail = 0;//mem_baseÖÐÊ£ÓàµÄ¿ÉÓÃÄÚ´æ

/**
 * Access to the slab allocator is protected by this lock
 */
 //Õâ¸öËøÓÃÀ´±£»¤¶Ôslab allocatorµÄ·ÃÎÊ
 ...
 */
 //slabs.c
 //³õÊ¼»¯slabclass£¬Èç¹ûprealloc£¬ÄÇÃ´Ô¤ÏÈÎªÃ¿¸öslabclass·ÖÅäÒ»Ð©¿ÕÏÐµÄitem
void slabs_init(const size_t limit, const double factor, const bool prealloc, const uint32_t *slab_sizes) {
    int i = POWER_SMALLEST - 1;//Êµ¼ÊµÈÓÚ0
    unsigned int size = sizeof(item) + settings.chunk_size;//³ß´çµÈÓÚchunkµÄ´óÐ¡¼ÓÉÏ¶îÍâÐÅÏ¢µÄ´óÐ¡

    mem_limit = limit;

     if (prealloc) { /* Allocate everything in a big chunk with malloc *///Ê¹ÓÃmallocÔÚÒ»¸ö´óµÄchunk ÖÐ·ÖÅäËùÓÐ¿Õ¼ä
    ...
    memset(slabclass, 0, sizeof(slabclass));//½«slabclassÁÐ±íÇå¿Õ

    while (++i < MAX_NUMBER_OF_SLAB_CLASSES-1) {//Õë¶ÔÃ¿Ò»¸öslabclass½øÐÐ³õÊ¼»¯£¬´Ó1¿ªÊ¼£¬0ºÅÓÐÆäËûÓÃ´¦
        if (slab_sizes != NULL) {//Èç¹ûÖ¸¶¨ÁËÔªËØµÄsize£¬ÄÇÃ´Ê¹ÓÃÖ¸¶¨µÄÖµ
            if (slab_sizes[i-1] == 0)
                break;
            size = slab_sizes[i-1];//ÕâÀïÊ¹ÓÃi - 1
        } else if (size >= settings.slab_chunk_size_max / factor) {//ÔÙ²»Ê¹ÓÃÊÖ¶¯ÅäÖÃÄÚ´æµÄÇé¿öÏÂ³¬¹ýÏÞÖÆ
            break;
        }
		
        if (size % CHUNK_ALIGN_BYTES)//½«size ½øÐÐ 8×Ö½Ú¶ÔÆë
            size += CHUNK_ALIGN_BYTES - (size % CHUNK_ALIGN_BYTES);

        slabclass[i].size = size;//¶ÔÆëÖ®ºóµÄ³ß´ç
        slabclass[i].perslab = settings.slab_page_size / slabclass[i].size;//´æ·ÅµÄÔªËØÊýÁ¿?
        if (slab_sizes == NULL)
            size *= factor;
        if (settings.verbose > 1) {//Èç¹û´ò¿ªµ÷ÊÔÐÅÏ¢
    ...
    if (prealloc) {
        slabs_preallocate(power_largest);//ÎªÃ¿Ò»¸öslabclass ½øÐÐÔ¤·ÖÅä
    }
}
...
static void slabs_preallocate (const unsigned int maxslabs) {
    ...
    /* pre-allocate a 1MB slab in every size class so people don't get
       ...
       these three lines.  */
       //ÔÚ²»Í¬sizeµÄclassÖÐÔ¤·ÖÅäÒ»¸ö1 MB µÄslab
...
static int grow_slab_list (const unsigned int id) {
    ...
    if (p->slabs == p->list_size) {
        ...
        void *new_list = realloc(p->slab_list, new_size * sizeof(void *));//À©´óÖ¸ÕëÊý×é
...
//½«ÐÂ·ÖÅäµÄslab page·Ö¸î³Éitem²¢³õÊ¼»¯ÀïÃæµÄÄÚÈÝ
...
//slabs.c
//ÔÚslabs_preallocate ÖÐµ÷ÓÃ£¬Îªslabclasses[id]½øÐÐÔ¤·ÖÅä
//·ÖÅäÒ»¸öÐÂµÄslab£¬¼´chunkµÄ¼¯ºÏ
static int do_slabs_newslab(const unsigned int id) {
    slabclass_t *p = &slabclass[id];
    slabclass_t *g = &slabclass[SLAB_GLOBAL_PAGE_POOL];
    int len = (settings.slab_reassign || settings.slab_chunk_size_max != settings.slab_page_size)
        ? settings.slab_page_size
        : p->size * p->perslab;//ÕâÀïµÄlenÊ²Ã´ÒâË¼?
    char *ptr;

    if ((mem_limit && mem_malloced + len > mem_limit && p->slabs > 0
         && g->slabs == 0)) {//ÕâÀïÅÐ¶ÏÊÇ·ñ»á³¬³öÄÚ´æÏÞÖÆ
        mem_limit_reached = true;//´ïµ½ÄÚ´æÏÞÖÆ
    ...
    split_slab_page_into_freelist(ptr, id);//½«ÐÂ·ÖÅäµÄslab page ²ð·Ö³Éitem·Åµ½slotsÖÐ

    p->slab_list[p->slabs++] = ptr;//¸üÐÂ¸Ãslabclass µÄslab_list
...
static void *do_slabs_alloc(const size_t size, unsigned int id, uint64_t *total_bytes,
    ...
    if (id < POWER_SMALLEST || id > power_largest) {//¶ÔidµÄ·¶Î§½øÐÐ¼ì²é
        MEMCACHED_SLABS_ALLOCATE_FAILED(size, 0);
        return NULL;
    }
    p = &slabclass[id];//»ñµÃ¶ÔslabclassµÄÖ¸Õë
    assert(p->sl_curr == 0 || ((item *)p->slots)->slabs_clsid == 0);
    if (total_bytes != NULL) {
        *total_bytes = p->requested;//total_bytes·µ»Øslabclass_tµÄrequestedÊýÁ¿
    }

    if (size <= p->size) {
		//Ê×ÏÈ´Ó×î½ü·ÖÅäµÄÒ³ÖÐ²éÕÒ¿Õ¼ä£¬È»ºóÊÇ¿ÕÏÐÁ´±í£¬È»ºóÊÇ·ÖÅäÐÂ¿Õ¼ä
        /* fail unless we have space at the end of a recently allocated page,
           we have something on our freelist, or we could allocate a new page */
        if (p->sl_curr == 0 && flags != SLABS_ALLOC_NO_NEWPAGE) {
            do_slabs_newslab(id);//´´½¨ÐÂµÄslab£¬²¢ÇÒ°Ñ¿Õ¼äÌí¼Óµ½¿ÕÏÐÁ´±íµÄÎ²²¿
        }

        if (p->sl_curr != 0) {//´Ó¿ÕÏÐÁ´±íÖÐ²éÕÒ¿Õ¼ä
            ...
            it->it_flags &= ~ITEM_SLABBED;//È¥³ýitemµÄslabbedÊôÐÔ
            it->refcount = 1;
            p->sl_curr--;
            ret = (void *)it;
        } else {
...
static void do_slabs_free_chunked(item *it, const size_t size, unsigned int id,
                                  slabclass_t *p) {
    item_chunk *chunk = (item_chunk *) ITEM_data(it);//item_chunkÊÇitemµÄÒ»²¿·Öå
    size_t realsize = size;
    while (chunk) {
        realsize += sizeof(item_chunk);//ÕâÀïÖ÷ÒªÊÇ¼ÆËãrealsize
        chunk = chunk->next;//ÕâÀïµÄ½á¹¹ºÜÆæ¹Ö£¬ÄÑµÀÊÇitem_chunkÒÔÊý×é¶ø²»ÊÇÖ¸Õë´æ´¢
    }
    chunk = (item_chunk *) ITEM_data(it);
    unsigned int chunks_found = 1;

    it->it_flags = ITEM_SLABBED;//ÕâÀïÎªÊ²Ã´Òª°ÑÊý¾ÝÈç´ËÉèÖÃå
    it->slabs_clsid = 0;
    it->prev = 0;
...
//slabs.c
//Çå¿Õitem ÖÐµÄÐÅÏ¢£¬½«Æä·Åµ½slotsÖÐ
static void do_slabs_free(void *ptr, const size_t size, unsigned int id) {
    slabclass_t *p;
    item *it;
...
//slabs.c
//ÄÚ´æ·ÖÅä£¬Èç¹ûÊ¹ÓÃÁËÔ¤·ÖÅä£¬Ôò´ÓÔ¤·ÖÅäµÄ¿Õ¼äÖÐ·ÖÅä£¬·ñÔòÏòÏµÍ³ÉêÇë
static void *memory_allocate(size_t size) {
    void *ret;
	...
	//8×Ö½Ú¶ÔÆë£¬²»¿ÉÄÜ³öÏÖ¶ÔÆëÖ®Ç°Î´³¬¹ý£¬µ«ÊÇ¶ÔÆëÖ®ºó³¬¹ýµÄÇé¿ö
	//ÒòÎª¶¼ÊÇ8×Ö½Ú¶ÔÆëµÄ
