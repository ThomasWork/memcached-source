...
volatile int slab_rebalance_signal;//（1）告诉编译器不做优化，（2）每次从内存中读取该值，而不是利用cache或寄存器中的值
//具体volatile的作用可以参见网址：http://blog.csdn.net/tigerjibo/article/details/7427366

/** file scope variables **/
//文件范围内的变量
static conn *listen_conn = NULL;//正在监听的连接队列
static int max_fds;
static struct event_base *main_base;//主循环
 ...
 */
static volatile bool allow_new_conns = true;//是否允许新的连接进入，
									//当关闭一个连接时，该值被设置为真
									//所以，如果连接数目达到了最大值，该值为假?
 ...
 * be that low).
 */
 //传给它的参数要么是unix时间，要么是距离当前时间的间隔
 //返回unix时间，使用的依据是delta不可能超过一个月
...
static void *conn_timeout_thread(void *arg) {
    ...
    while(1) {
        ...
        for (i = 0; i < max_fds; i++) {
            ...
            if (!IS_TCP(c->transport))//如果是TCP连接
                continue;
...
//直到需要连接时我们才分配连接空间
//但是这里提前分配了内存来保存指针，
//优点是可以能够直接使用FD来索引连接数组
static void conn_init(void) {
    /* We're unlikely to see an FD much higher than maxconns. */
    int next_fd = dup(1);//复制成功时返回最小的尚未被使用的文件描述符
...
conn *conn_new(const int sfd, enum conn_states init_state,
    ...
    if (NULL == c) {//如果为空，表示
        ...
        MEMCACHED_CONN_CREATE(c);//后面开始初始化连接信息
        ...
        c->sfd = sfd;//这里让连接和文件描述符对应
        conns[sfd] = c;
    }

    c->transport = transport;
    c->protocol = settings.binding_protocol;//这个字段是否是每次新建一个连接时都更新?
    ...
    c->last_cmd_time = current_time; //当前时间为一个全局变量，一秒更新一次 /* initialize for idle kicker */
...
static void conn_cleanup(conn *c) {
    ...
    conn_release_items(c);//清空连接里面的items，但是这些items到底有什么用?
...
static void conn_close(conn *c) {
    ...
    event_del(&c->event);//删除时间，那么来自该连接的消息都将收不到了

    if (settings.verbose > 1)
        fprintf(stderr, "<%d connection closed.\n", c->sfd);

    conn_cleanup(c);//清空conn中的内容

    MEMCACHED_CONN_RELEASE(c->sfd);
    conn_set_state(c, conn_closed);//将状态设置为关闭
    close(c->sfd);//关闭句柄
...
static int ensure_iov_space(conn *c) {
    ...
    if (c->iovused >= c->iovsize) {
        ...
        struct iovec *new_iov = (struct iovec *)realloc(c->iov,
                                (c->iovsize * 2) * sizeof(struct iovec));//每次翻倍
        if (! new_iov) {
            STATS_LOCK();
            stats.malloc_fails++;
            STATS_UNLOCK();
...
static int add_iov(conn *c, const void *buf, int len) {
    ...
    do {
        ...
        if (ensure_iov_space(c) != 0)//增加iov数组的空间
            return -1;
...
static void out_of_memory(conn *c, char *ascii_error) {
    ...
    if (c->protocol == binary_prot) {
        ...
        write_bin_error(c, PROTOCOL_BINARY_RESPONSE_ENOMEM, ascii_error, 0);//这里会设置状态
    } else {
        out_string(c, ascii_error);//设置状态以及write_and_go
    }
}
 ...
 */
 //我们走到这一步说明我们已经读取了set/add/replace命令中的值。
 //命令已经被存储在c->cmd中，item已经存储在c->item中
...
//下面判断is_valid是否为真
    if ((it->it_flags & ITEM_CHUNKED) == 0) {//表示item不是组合的
    ...
    if (!is_valid) {
      ...
      ret = store_item(it, comm, c);//////////////////////////////////////这里是实际的业务逻辑
...
static void write_bin_response(conn *c, void *d, int hlen, int keylen, int dlen) {
    if (!c->noreply || c->cmd == PROTOCOL_BINARY_CMD_GET ||
        c->cmd == PROTOCOL_BINARY_CMD_GETK) {//在这种情况下需要回复信息
 ...
 */
 //在store_item函数中已经对item加锁了，然后才调用的它
enum store_item_type do_store_item(item *it, int comm, conn *c, const uint32_t hv) {
    char *key = ITEM_key(it);
    item *old_it = do_item_get(key, it->nkey, hv, c);//从hash表中查找元素，如果过期，设置为NULL
        ...
        || comm == NREAD_APPEND || comm == NREAD_PREPEND))
    {
        ...
        if(old_it == NULL) {
            ...
            stored = NOT_FOUND;
            pthread_mutex_lock(&c->thread->stats.mutex);
            c->thread->stats.cas_misses++;//更新线程中的找不到技计数
            pthread_mutex_unlock(&c->thread->stats.mutex);
        }
        else if (ITEM_get_cas(it) == ITEM_get_cas(old_it)) {//找到之前存在的，并且cas属性一致
            // cas validates
            // it and old_it may belong to different classes.
            // I'm updating the stats for the one that's getting pushed out
            pthread_mutex_lock(&c->thread->stats.mutex);
            c->thread->stats.slab_stats[ITEM_clsid(old_it)].cas_hits++;//更新线程统计信息
            pthread_mutex_unlock(&c->thread->stats.mutex);

            item_replace(old_it, it, hv);//替换操作?????????????????????????????????这里执行的实际上是替换?
            stored = STORED;
        } else {//找到了，但是两者cas属性不一致，所以进行计数后什么都不做
            pthread_mutex_lock(&c->thread->stats.mutex);
    ...
    if (old_it != NULL)
        do_item_remove(old_it);   //将其挂载 到slots中
    if (new_it != NULL)
    ...
    do {
        while(key_token->length != 0) {
            ...
            if(nkey > KEY_MAX_LENGTH) {
                out_string(c, "CLIENT_ERROR bad command line format");
                while (i-- > 0) {
                    item_remove(*(c->ilist + i));//清空连接里面保存的item
                }
                return;
            }
            ...
            if (it) {
                ...
                else
                {
                  ...
                  if ((it->it_flags & ITEM_CHUNKED) == 0)//不是chunked数据
                      {
				...
				//item_get增加了it->refcount
                pthread_mutex_lock(&c->thread->stats.mutex);
                c->thread->stats.slab_stats[ITEM_clsid(it)].get_hits++;
                c->thread->stats.get_cmds++;
                pthread_mutex_unlock(&c->thread->stats.mutex);
                item_update(it);//////////////////////////////////////////更新item的访问时间，非频繁更新
                *(c->ilist + i) = it;
                i++;
...
static void process_update_command(conn *c, token_t *tokens, const size_t ntokens, int comm, bool handle_cas) {
    ...
    set_noreply_maybe(c, tokens, ntokens);//设置是否需要回复?
    ...
    if (! (safe_strtoul(tokens[2].value, (uint32_t *)&flags)//读取flags
           && safe_strtol(tokens[3].value, &exptime_int)//读取exptime_int
           && safe_strtol(tokens[4].value, (int32_t *)&vlen))) {//读取vlen
        out_string(c, "CLIENT_ERROR bad command line format");
        return;
    }
    ...
    vlen += 2;//这里加了2，是为什么呢?为了多分配一点数据?

    if (settings.detail_enabled) {
        stats_prefix_record_set(key, nkey);
    }

    it = item_alloc(key, nkey, flags, realtime(exptime), vlen);/////////////////////这里分配了空间
    											//但是实际上数据好像没有存储进去

    if (it == 0) {
        enum store_item_type status;
        if (! item_size_ok(nkey, flags, vlen)) {//目标太大了
        ...
        /* swallow the data line */
        c->write_and_go = conn_swallow;//////////////////////////////////////////////////////状态转换
        c->sbytes = vlen;

        /* Avoid stale data persisting in cache because we failed alloc.
         * Unacceptable for SET. Anywhere else too? */
        if (comm == NREAD_SET) {//如果命令是set
            it = item_get(key, nkey, c);
            if (it) {
                item_unlink(it);//如果查找到该元素，则把它删除
                item_remove(it);//前一个函数已经调用了do_item_remove，为什么是2次呢?
            }
        }
...
static void process_command(conn *c, char *command) {
    ...
    c->iovused = 0;
    if (add_msghdr(c) != 0) {
        out_of_memory(c, "SERVER_ERROR out of memory preparing response");//内存错误，(先写)然后到new_conn状态
        return;
    }

    ntokens = tokenize_command(command, tokens, MAX_TOKENS);//将命令串按空格分割
    if (ntokens >= 3 &&
        ((strcmp(tokens[COMMAND_TOKEN].value, "get") == 0) ||// COMMAND_TOKEN 实际上等于0
         (strcmp(tokens[COMMAND_TOKEN].value, "bget") == 0))) {

        process_get_command(c, tokens, ntokens, false);//处理                           get                        命令
        ...
        process_update_command(c, tokens, ntokens, comm, false);// 处理      update                  命令
        ...
        if (ntokens != (c->noreply ? 3 : 2)) {
            exptime = strtol(tokens[1].value, NULL, 10);//过期时间
            if(errno == ERANGE) {
                out_string(c, "CLIENT_ERROR bad command line format");
                return;
            }
        ...
        */
        //如果exptime 等于0，那么realtime() 也会返回0
        //那么0-1会溢出得到自大的无符号值
        //
        if (exptime > 0) {
            new_oldest = realtime(exptime);
...
static enum try_read_result try_read_udp(conn *c) {
    ...
    res = recvfrom(c->sfd, c->rbuf, c->rsize,
                   ...
                   &c->request_addr_size);
    if (res > 8) {//这里为什么要大于8呢，8是随便指定的一个大于0的数字?
 ...
 */
 //尽可能地多从网络中读取，处理缓存溢出以及连接关闭
 //在读取之前，把一个命令的剩下的段移动到缓存的头部
 //为了保护我们免受某人使用伪造的数据flooding连接，造成连接吃掉所有可用内存
 //，跳出并且在几次realloc 之后开始检查刚刚接收到的数据
static enum try_read_result try_read_network(conn *c) {
    ...
    if (c->rcurr != c->rbuf) {
        if (c->rbytes != 0) /* otherwise there's nothing to copy */
            memmove(c->rbuf, c->rcurr, c->rbytes);//可以处理内存覆盖的情况，dest, src, n
        c->rcurr = c->rbuf;
    }

    while (1) {
        if (c->rbytes >= c->rsize) {
            if (num_allocs == 4) {//如果已经达到分配次数上限
                return gotdata;
            }
            ++num_allocs;
            char *new_rbuf = realloc(c->rbuf, c->rsize * 2);//分配更大的空间
            if (!new_rbuf) {
                STATS_LOCK();
                stats.malloc_fails++;
                STATS_UNLOCK();
                ...
                c->write_and_go = conn_closing;////////注意这里有一个状态转变
                return READ_MEMORY_ERROR;
            }
            c->rcurr = c->rbuf = new_rbuf;//rcurr 只是一个指针，指向当前缓存的某个位置
            c->rsize *= 2;
        }

        int avail = c->rsize - c->rbytes;//可以读入这么多内容
        res = read(c->sfd, c->rbuf + c->rbytes, avail);
        if (res > 0) {
            pthread_mutex_lock(&c->thread->stats.mutex);//这里更新线程内部状态是否有必要加锁呢?
            c->thread->stats.bytes_read += res;
            pthread_mutex_unlock(&c->thread->stats.mutex);
            gotdata = READ_DATA_RECEIVED;
            c->rbytes += res;
            if (res == avail) {
                continue;//空间读满了，需要扩容，继续读
            } else {
                break;//空间没有读满，内容以及读完了
            }
        }
        if (res == 0) {
            return READ_ERROR;
...
static enum transmit_result transmit(conn *c) {
    ...
    if (c->msgcurr < c->msgused &&
            c->msglist[c->msgcurr].msg_iovlen == 0) {//当前消息写完毕，推荐到下一个消息
        /* Finished writing the current msg; advance to the next. */
        c->msgcurr++;//
    }
    if (c->msgcurr < c->msgused) {
        ssize_t res;
        struct msghdr *m = &c->msglist[c->msgcurr];//指向当前的msg

        res = sendmsg(c->sfd, m, 0);//发送消息
        if (res > 0) {//发送成功
            pthread_mutex_lock(&c->thread->stats.mutex);
            ...
            /* We've written some of the data. Remove the completed
               ...
               //我们已经写了部分数据，从等待写的列表中移除完成的iovec实体
...
static void drive_machine(conn *c) {
    ...
    while (!stop) {
        ...
        switch(c->state) {
        case conn_listening://即使不读取，那么下次也会读取的
            addrlen = sizeof(addr);
#ifdef HAVE_ACCEPT4
            if (use_accept4) {
            ...
            if (sfd == -1) {
                ...
                if (errno == EAGAIN || errno == EWOULDBLOCK) {
                    /* these are transient, so don't log anything *///短暂的，临时的
                    stop = true;
        ...
        case conn_waiting:
            if (!update_event(c, EV_READ | EV_PERSIST)) {//更新事件，如果一致则不用更新
                                                                                      //这里应该主要是为了反馈那个事件
                if (settings.verbose > 0)
                    fprintf(stderr, "Couldn't update event\n");
                conn_set_state(c, conn_closing);
            ...
            conn_set_state(c, conn_read);
            stop = true;//这里stop为true，会跳出去，但是该监听事件仍然成立
            break;
        ...
        case conn_read:
            ...
            switch (res) {
            case READ_NO_DATA_RECEIVED://第一次读取发生中断可能会这样
                conn_set_state(c, conn_waiting);
                break;
            case READ_DATA_RECEIVED:
                conn_set_state(c, conn_parse_cmd);
                break;
            case READ_ERROR://应该是关闭连接
                conn_set_state(c, conn_closing);
                break;
        ...
        case conn_parse_cmd ://注意，这里下一步循环仍然继续进行
        ...
        case conn_new_cmd://注意，这里没有设置为true，所以循环会继续进行下去
            /* Only process nreqs at a time to avoid starving other
               connections */
               //一次仅仅处理n条请求，以防止其他连接饥饿

            --nreqs;
            if (nreqs >= 0) {
                reset_cmd_handler(c);//还可以完成部分请求
            } else {//如果请求执行完成
                pthread_mutex_lock(&c->thread->stats.mutex);
                ...
                if (c->rbytes > 0) {
                    ...
                    */
                    if (!update_event(c, EV_WRITE | EV_PERSIST)) {//那么应该会排在队列尾部，
                        if (settings.verbose > 0)                                     //下次触发写事件时仍然可以处理命令
                                                                                                  //但是下次得找机会改过来
                            fprintf(stderr, "Couldn't update event\n");
                        conn_set_state(c, conn_closing);
                        break;
        ...
        case conn_nread://这里应该已经开始执行命令了
            if (c->rlbytes == 0) {//这里应该说的是: 如果没有需要读取的内容
                complete_nread(c);
                break;
            }
		...
		//如果item为空，或者数据不是chunked
            if (!c->item || (((item *)c->item)->it_flags & ITEM_CHUNKED) == 0) {
                /* first check if we have leftovers in the conn_read buffer */
                if (c->rbytes > 0) {
                    int tocopy = c->rbytes > c->rlbytes ? c->rlbytes : c->rbytes;//需要拷贝的数据长度
                    if (c->ritem != c->rcurr) {
                        memmove(c->ritem, c->rcurr, tocopy);//ritem实际指向存储数据的空间
                    }
                    c->ritem += tocopy;//移动当前指针
                    c->rlbytes -= tocopy;//需要读取的长度更新
                    c->rcurr += tocopy;
                    c->rbytes -= tocopy;
                    if (c->rlbytes == 0) {//如果数据内容读取完毕
                        break;
                    }
                }

		//缓存中的数据读取完毕，现在开始从socket中读取
                ...
                if (res > 0) {
                    ...
                    c->ritem += res;
                    c->rlbytes -= res;
                    break;///这里跳出去了，但是如果数据没有读完会出发 EAGAIN
                }
            } else {
                res = read_into_chunked_item(c);/////////////读取chunked    item
                if (res > 0)
                    break;
            }

            if (res == 0) { /* end of stream */         //这里应该是数据读取完毕了，但是没有执行命令吗
                conn_set_state(c, conn_closing);
                break;
            }

            if (res == -1 && (errno == EAGAIN || errno == EWOULDBLOCK)) {//如果读取被中断，跳出循环
	...
	//首先检查在缓存中的数据
        ...
        case conn_write:
             ...
             */
             //我们想向外写一个简单的回应，如果我们还没有完成
             //那么就把它组装进一个消息缓存列表
            if (c->iovused == 0 || (IS_UDP(c->transport) && c->iovused == 1)) {
                if (add_iov(c, c->wcurr, c->wbytes) != 0) {//这里主要是牵涉到底层的实现
                    if (settings.verbose > 0)
          ...
          if (IS_UDP(c->transport) && c->msgcurr == 0 && build_udp_headers(c) != 0) {
            ...
            case TRANSMIT_COMPLETE:
                if (c->state == conn_mwrite) {
                    ...
                    if(c->protocol == binary_prot) {
                        conn_set_state(c, c->write_and_go);//为什么二进制协议要这样呢?
                    } else {
                        conn_set_state(c, conn_new_cmd);
                    }
            ...
            case TRANSMIT_INCOMPLETE://没有把一条消息写完
            case TRANSMIT_HARD_ERROR:
                break;                   //在状态机中继续执行

            case TRANSMIT_SOFT_ERROR://停止状态机
                stop = true;
                break;
            }
            break;//再次进入循环
        ...
        case conn_closed:
            /* This only happens if dormando is an idiot. */ //如果dormando是个傻瓜
            abort();
            break;
 ...
 *        listen on.
 */
 //创建一个socket 并把它绑定到特定的端口号
static int server_socket(const char *interface,
                         int port,
                         enum network_transport transport,//传输协议
                         FILE *portnumber_file) {//一个文件，用来写端口号码
    int sfd;
...
//设置调用close仍然可以重新使用该socket
        setsockopt(sfd, SOL_SOCKET, SO_REUSEADDR, (void *)&flags, sizeof(flags));//到这里应该都是1
        if (IS_UDP(transport)) {
            maximize_sndbuf(sfd);
        } else {//设置 TCP 属性
            ...
            error = setsockopt(sfd, SOL_SOCKET, SO_LINGER, (void *)&ling, sizeof(ling));//经历time_wait的过程
            if (error != 0)
                perror("setsockopt");
        ...
        if (bind(sfd, next->ai_addr, next->ai_addrlen) == -1) {
            ...
            success++;
            if (!IS_UDP(transport) && listen(sfd, settings.backlog) == -1) {//开始TCP  监听，这里是等于-1
                perror("listen()");
                close(sfd);
                freeaddrinfo(ai);
                return 1;
        ...
        if (IS_UDP(transport)) {
            ...
            if (!(listen_conn_add = conn_new(sfd, conn_listening,//新建连接对象，并把它绑定到base中
                                             EV_READ | EV_PERSIST, 1,
                                             transport, main_base))) {
                fprintf(stderr, "failed to create listening connection\n");
                exit(EXIT_FAILURE);
            }
            listen_conn_add->next = listen_conn;//加入连接队列
...
static int server_sockets(int port, enum network_transport transport,
    ...
    if (settings.inter == NULL) {
        ...
        char *b;
        int ret = 0;
        char *list = strdup(settings.inter);//拷贝字符串，最后需要使用free释放
 ...
 */
 //我们 在全局变量中保存当前时间，并且使用一个计时事件更新它。
 //这为我们节省了很多时间，因为我们实际上只需要1秒钟更新一次时间。
volatile rel_time_t current_time;
static struct event clockevent;

//libevent使用单调时间进行事件调度，除了jitter?
/* libevent uses a monotonic clock when available for event scheduling. Aside
 * from jitter, simply ticking our internal timer here is accurate enough.
 * Note that users who are setting explicit dates for expiration times *must*
 * ensure their clocks are correct before starting memcached. */
 //请注意如果用户为过期时间设置明确的日期，
 //必须在启动memcached之前确保他们的时钟是准确的
static void clock_handler(const int fd, const short which, void *arg) {
    struct timeval t = {.tv_sec = 1, .tv_usec = 0};//1秒钟更新一次
    ...
    evtimer_set(&clockevent, clock_handler, 0);//设置事件
    event_base_set(main_base, &clockevent);//设置与之关联的base
    evtimer_add(&clockevent, &t);//添加事件
...
//可以用来处理中断信号，strsignal返回信号的字符串描述符，在下次调用该函数时字符串失效
 ...
 */
 //memcached.c
 //对运行时环境进行基本的可用性测试，如果没有错误返回true，否则不能使用该环境
...
int main (int argc, char **argv) {
    ...
    if (!sanitycheck()) {//可用性测试,通过测试则返回true
        return EX_OSERR;
    }

    /* handle SIGINT and SIGTERM *///对信号进行处理
    signal(SIGINT, sig_handler);//中断信号，ctrl+c
    signal(SIGTERM, sig_handler);//kill命令发出的信号

    settings_init();//对程序运行需要的参数进行初始化
...
这里就不知道他想表达什么了
    */
    ...
    setbuf(stderr, NULL);//为stderr指定缓冲，这里设置为NULL，表示直接输出
...
//设置hash算法
     ...
     * multiple ports
     */
     //如果用户指定了多个端口，则令一个工作线程服务每一个UDP端口
    ...
    if (maxcore != 0) {//这里的maxcore什么意思?
        struct rlimit rlim_new;
        /*
         * First try raising to infinity; if that fails, try bringing
         * the soft limit to the hard.
         */
        if (getrlimit(RLIMIT_CORE, &rlim) == 0) {//RLIMIT_CORE表示内核转存文件的最大长度
            rlim_new.rlim_cur = rlim_new.rlim_max = RLIM_INFINITY;//RLIM_INFINITY表示不对资源进行限制
...
//如果有需要，把资源限制增加来允许接收更多连接
    if (getrlimit(RLIMIT_NOFILE, &rlim) != 0) {//比进程可打开的最大描述符数大1，超过此值将会产生EMFILE
    ...
    if (lock_memory) {
#ifdef HAVE_MLOCKALL
        int res = mlockall(MCL_CURRENT | MCL_FUTURE);//锁定内存，禁止进行交换
        if (res != 0) {
    ...
    slabs_init(settings.maxbytes, settings.factor, preallocate,
     ...
     //忽略PIPE信号，PIPE信号是当网络连接一端已经断开，
     //这时发送数据，会进行RST的重定向，再次发送数据，
     //会触发PIPE信号，而PIPE信号的默认动作是退出进程，
     //所以需要忽略该信号
    ...
    if (start_lru_crawler && start_item_crawler_thread() != 0) {//这个线程是用来做什么的?
        fprintf(stderr, "Failed to enable LRU crawler thread\n");
        exit(EXIT_FAILURE);
    }

    if (start_lru_maintainer && start_lru_maintainer_thread() != 0) {//LRU线程是用来做什么的?
        fprintf(stderr, "Failed to enable LRU maintainer thread\n");
        return 1;
    }

    if (settings.slab_reassign &&
        start_slab_maintenance_thread() == -1) {//这个线程是用来?
        exit(EXIT_FAILURE);
    }

    if (settings.idle_timeout && start_conn_timeout_thread() == -1) {//连接超时检测线程
        exit(EXIT_FAILURE);
    }

    clock_handler(0, 0, 0);//初始化计时事件，这样事件每秒钟更新一次，防止系统调用
     ...
     * is only an advisory.//顾问的，劝告的
     */
    usleep(1000);
    if (stats_state.curr_conns + stats_state.reserved_fds >= settings.maxconns - 1) {//保留描述符
        fprintf(stderr, "Maxconns setting is too low, use -c to increase.\n");
        exit(EXIT_FAILURE);
    }

    if (pid_file != NULL) {
        save_pid(pid_file);//保存进程id?
    }
