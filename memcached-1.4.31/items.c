...
static unsigned int sizes[LARGEST_ID];//LRU 队列中有多少元素
static uint64_t sizes_bytes[LARGEST_ID];// LRU 队列总共有多少内存
static unsigned int *stats_sizes_hist = NULL;//保存的是占有不同(32*n)字节的item的数量?
...
item *do_item_alloc(char *key, const size_t nkey, const unsigned int flags,
    ...
    int i;
    uint8_t nsuffix;
    item *it = NULL;//suffix中存储的是flags和nbytes-2，实际是什么内容呢?
    char suffix[40];//这里的后缀空间先开大一点，在下一步会收缩。
    size_t ntotal = item_make_header(nkey + 1, flags, nbytes, suffix, &nsuffix);//计算item的总空间大小
    if (settings.use_cas) {//如果使用cas则将相应的空间增加
        ntotal += sizeof(uint64_t);
    }

    unsigned int id = slabs_clsid(ntotal);//这里为找到合适的clabclass，但是最大的slabclass的
    								//size都小于所需的空间会怎么办?
    								//chunked数据是怎么回事?
    if (id == 0)
        return 0;
     ...
     * and replacing small items.
     //如果没有可用内存
     */
    for (i = 0; i < 10; i++) {//循环进行10次
        uint64_t total_bytes;
        /* Try to reclaim memory first */// 首先尝试取回内存
        if (!settings.lru_maintainer_thread) {//如果没有设置该字段
            lru_pull_tail(id, COLD_LRU, 0, 0);//
        }
        it = slabs_alloc(ntotal, id, &total_bytes, 0);//这里即为为新的元素分配空间，ntoal可能大于对应id
        //的size，那么需要分配chunked item  ?

        if (settings.expirezero_does_not_evict)
            total_bytes -= noexp_lru_size(id);

        if (it == NULL) {//如果分配新空间失败，则进行下面的步骤
     ...
     */
    if (settings.lru_maintainer_thread) {//这里对id做的处理什么意思?莫非是要更新id?
    ...
    it->exptime = exptime;
    memcpy(ITEM_suffix(it), suffix, (size_t)nsuffix);//这里nsuffix已经更新为实际需要的大小
    it->nsuffix = nsuffix;
...
int do_item_link(item *it, const uint32_t hv) {
    ...
    item_link_q(it);//加锁并加入到lru队列中
    refcount_incr(&it->refcount);//增加引用计数，莫非加入到hash表中就算一次引用?
    item_stats_sizes_add(it);

    return 1;
}

//item_unlink 加锁之后调用该函数，从hash表中删除，从LRU队列删除，加入slots
void do_item_unlink(item *it, const uint32_t hv) {
    MEMCACHED_ITEM_UNLINK(ITEM_key(it), it->nkey, it->nbytes);
    if ((it->it_flags & ITEM_LINKED) != 0) {//这里是为了防止多次unlink?
        it->it_flags &= ~ITEM_LINKED;//设置item为非linked
        STATS_LOCK();
        stats_state.curr_bytes -= ITEM_ntotal(it);
        stats_state.curr_items -= 1;
        STATS_UNLOCK();
        ...
        item_unlink_q(it);//从LRU队列中删除元素
        do_item_remove(it);//如果refcount减到0，则把它挂载到slots中
    }
}
...
void do_item_unlink_nolock(item *it, const uint32_t hv) {
    ...
    if ((it->it_flags & ITEM_LINKED) != 0) {
        ...
        item_stats_sizes_remove(it);
        assoc_delete(ITEM_key(it), it->nkey, hv);//从hash表中删除
        do_item_unlink_q(it);//从 LRU队列中删除
        do_item_remove(it);//如果item的refcount等于0，将item移动到slots中，
    }
}

//如果item的引用计数减少到0，将其放到slab的slots中
...
void do_item_update(item *it) {
    ...
    if (it->time < current_time - ITEM_UPDATE_INTERVAL) {//如果更新时间 大于60秒
        ...
        if ((it->it_flags & ITEM_LINKED) != 0) {//如果它在链表中
            it->time = current_time;
            if (!settings.lru_maintainer_thread) {//lru 保持线程专门调整顺序?
                item_unlink_q(it);
                item_link_q(it);//从头部插入
            }
        }
    }
}
...
int do_item_replace(item *it, item *new_it, const uint32_t hv) {
    ...
    do_item_unlink(it, hv);//三步删除，不加锁
    return do_item_link(new_it, hv);//不加锁添加到hash表和LRU
}
 ...
 */
 //我认为如果不使用CAS值，没有什么方式来准确完成这个功能
 //
void item_stats_sizes_remove(item *it) {
    if (stats_sizes_hist == NULL || stats_sizes_cas_min > ITEM_get_cas(it))
        return;
    int ntotal = ITEM_ntotal(it);//item总共有多少字节
    int bucket = ntotal / 32;//是32字节的多少倍
    if ((ntotal % 32) != 0) bucket++;//向上取整数
    if (bucket < stats_sizes_buckets) stats_sizes_hist[bucket]--;
}
...
//对assoc_find 进行包装，它内部实现了延迟过期逻辑
item *do_item_get(const char *key, const size_t nkey, const uint32_t hv, conn *c) {
    item *it = assoc_find(key, nkey, hv);//这个函数纯粹用来从hash表中找到元素
    if (it != NULL) {
        refcount_incr(&it->refcount);//在该函数中，如果系统提供原子操作则利用，否则加锁
        /* Optimization for slab reassignment. prevents popular items from
         * jamming in busy wait. Can only do this here to satisfy lock order
         * of item_lock, slabs_lock. */

//对slab重新分配进行优化，阻止热门item 被挤进繁忙的等待中
//只能在这里这样做来满足加锁顺序(先 item lock，然后是slabs_lock)
    ...
    int was_found = 0;//这个变量用来保存是否找到元素以及该元素是否被清空等状态
    ...
    if (it != NULL) {
        was_found = 1;
        if (item_is_flushed(it)) {//根据oldesttime和cas判断是否清空该元素
            do_item_unlink(it, hv);
            do_item_remove(it);//可是 在do_item_unlink中已经调用了do_item_remove
            it = NULL;
            pthread_mutex_lock(&c->thread->stats.mutex);
            c->thread->stats.get_flushed++;//更新线程状态
            pthread_mutex_unlock(&c->thread->stats.mutex);
            if (settings.verbose > 2) {
                fprintf(stderr, " -nuked by flush");
            }
            was_found = 2;//被刷新掉
        } else if (it->exptime != 0 && it->exptime <= current_time) {//如果是0就不会过期?
            do_item_unlink(it, hv);
            do_item_remove(it);//????
            it = NULL;
            pthread_mutex_lock(&c->thread->stats.mutex);
            c->thread->stats.get_expired++;//更新线程的状态
            pthread_mutex_unlock(&c->thread->stats.mutex);
            if (settings.verbose > 2) {
 ...
 * Callable from worker threads or the LRU maintainer thread */
 //返回被移除的、过期的或者驱逐出的元素的数量
 //可以从worker线程或者LRU maintainer调用该函数
 //在command_update相关函数中调用了这个函数来清理空间?
static int lru_pull_tail(const int orig_id, const int cur_lru,
        const uint64_t total_bytes, uint8_t flags) {
    item *it = NULL;
    int id = orig_id;
    ...
    id |= cur_lru;//这里为什么要这样处理?
    pthread_mutex_lock(&lru_locks[id]);//首先对LRU队列加锁
    search = tails[id];//从尾巴开始搜索
    /* We walk up *only* for locked items, and if bottom is expired. */
    for (; tries > 0 && search != NULL; tries--, search=next_it) {
        /* we might relink search mid-loop, so search->prev isn't reliable */
        next_it = search->prev;
        if (search->nbytes == 0 && search->nkey == 0 && search->it_flags == 1) {//这样的item代表什么呢?
           //////////实际值为2
            if (flags & LRU_PULL_CRAWL_BLOCKS) { /* We are a crawler, ignore it. */// 退出程序
                pthread_mutex_unlock(&lru_locks[id]);
                return 0;
            }
            tries++;
        ...
        if ((hold_lock = item_trylock(hv)) == NULL)//如果加锁失败，继续循环
            continue;
        /* Now see if the item is refcount locked */
        if (refcount_incr(&search->refcount) != 2) {//到这里加锁成功，而且引用次数不为2
        ...
        switch (cur_lru) {
            ...
            case WARM_LRU:
                ...
                break;
            case COLD_LRU://worker线程调用该函数就是COLD_LRU
...
int init_lru_maintainer(void) {
    if (lru_maintainer_initialized == 0) {
        pthread_mutex_init(&lru_maintainer_lock, NULL);//初始化互斥锁
