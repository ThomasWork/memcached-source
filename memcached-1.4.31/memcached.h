...
#define ITEM_key(item) (((char*)&((item)->data)) \  //Ç¿ÖÆÀàÐÍ×ª»»Îªchar *
 ...
 */
 //memcached.h
 //ÔÚmemcachedÖÐ´æ´¢itemµÄ½á¹¹
typedef struct _stritem {
    /* Protected by LRU locks *///±»LRU Ëø±£»¤
    struct _stritem *next;
    struct _stritem *prev;
    /* Rest are protected by an item lock *///Ê£ÏÂµÄ±»Ò»¸öitem Ëø±£»¤
    struct _stritem *h_next;   //Ö¸ÏòhashÁ´±íÖÐµÄÏÂÒ»¸ö½Úµã
    rel_time_t      time;      //×î½ü·ÃÎÊÊ±¼ä /* least recent access */
    rel_time_t      exptime;   //¹ýÆÚÊ±¼ä 
    int             nbytes;    //Êý¾ÝµÄ´óÐ¡
    unsigned short  refcount;//ÒýÓÃ¼ÆÊý£¬Õâ¸öÔÚËøµÄÊ±ºòºÃÏñÓÐÃîÓÃ
    uint8_t         nsuffix;    /* length of flags-and-length string */
    uint8_t         it_flags;   //item µ±Ç°µÄ×´Ì¬£¬ÊÇ·ñÊÇlinked£¬chunkedµÈµÈ
    uint8_t         slabs_clsid; //´¦ÓÚÄÄ¸öslab class ÖÐ/* which slab class we're in */
    uint8_t         nkey;     //keyµÄ³¤¶È  /* key length, w/terminating null and padding */
    ...
    /* if it_flags & ITEM_CAS we have 8 bytes CAS *///Èç¹ûit_flags & ITEM_CAS£¬ÄÇÃ´¾ÍÊ¹ÓÃ8 bytes CAS£¬ÒÔ¼°ÒÔnull ½áÎ²µÄkey
    /* then null-terminated key */                                
    /* then " flags length\r\n" (no terminating null) */ //È»ºóÊÇ"flagg length\r\n"(Ã»ÓÐÖÕÖ¹·ûnull)£¬È»ºóÊÇÒÔ\r\n ½áÎ²µÄdata
...
typedef struct {
    ...
    struct event notify_event;  //¶ÔÓÚnotify_receive_fd µÄ¼àÌýÊÂ¼þ
    int notify_receive_fd;      //ÄäÃû¹ÜµÀµÄ¶Á¶Ë
    int notify_send_fd;         //ÄäÃû¹ÜµÀµÄÐ´¶Ë
    struct thread_stats stats;  //Õâ¸öÏß³ÌÉú³ÉµÄ×´Ì¬/* Stats generated by this thread */ÔÚmemcached.hÖÐ¶¨Òå
    struct conn_queue *new_conn_queue; //ÐÂµÄ½«Òª´¦ÀíµÄÁ¬½Ó¶ÓÁÐ£¬¶ÔÓÚÀÏµÄ¸ÃÔõÃ´°ìå
    cache_t *suffix_cache;     //Õâ¸öºó×ºcache?
    logger *l;            //logger »º´æ      /* logger buffer */
} LIBEVENT_THREAD;
...
struct conn {
    ...
    uint64_t cas; //½«Òª·µ»ØµÄcas    /* the cas to return */
